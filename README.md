...
### v3.18.22-22（2026-09-26）· 修复普通用户首页启停按钮状态不刷新
- 🔧 **普通用户启停按钮修复**：`/api/status` 的 running 字段——管理员返回全局网关状态，普通用户返回自己的 `api_enabled` 开关状态；点「我的API控制」按钮后按钮/状态/点灯立即正确切换（实测 qitong 用户：已停止→运行中 ✅）
- 🐛 **根因**：之前 `/api/status` 对所有用户返回全局 running，普通用户点自己的 API 开关后，首页重载显示的仍是全局状态 → 按钮"没反应"

### v3.18.22-21（2026-09-26）· 用户数据全面隔离 + 后台测速 + 友好错误（自主自检）
- 🔒 **测速排行榜按当前用户显示**：普通用户只看「公用+自己的」模型（管理员看全部），不再全部显示
- 🔒 **统计用量按当前用户**：`/api/stats` 模型汇总 / 传输明细 / 按密钥分组全部按当前登录用户隔离（管理员看全部，普通用户只看自己）
- ⚡ **后台批量测速**：离开页面也在跑，回来看进度/结果；新增 `/api/speedtest/start` + `/api/speedtest/progress`；前端进度条 + 轮询 + 结果回填
- 🎨 **首页UI重叠修复**：统计卡包进卡片容器 + grid-4 响应式降级（1200px→2列，768px→1列）
- 💬 **错误返回像AI回答**：全部上游失败时不再返回 5xx 报错，改为 HTTP 200 + 正常 chat completion 格式（流式也返回 SSE 提示），客户端显示对话内容
- 🔧 **测速权限修复**：普通用户可测速自己可见的模型（不再被 SYS_SPEED 权限挡住）
- 📊 **用量页新增「按API密钥用量」表**（对齐原APP apiKeyUsageRows）

### v3.18.22-20（2026-09-26）· P0邀请码修复 + P1限流防刷 + 安全加固（规划Agent自主）
- 🔧 **P0 邀请码 Bug 修复**：老用户空邀请码自动生成并持久化（QT+随机），个人中心与分销中心显示一致，邀请注册真正生效
- 🛡️ **P1 API 限流防刷**：新增 RateLimiter——每秒 QPS + 每日配额（按 Key + 用户隔离，管理员/本地不限），超出返回 429 中文提示；个人中心新增「限流设置」卡
- 🚨 **P1 暴力破解告警**：同 IP 连续 5 次登录失败自动触发钉钉/邮箱通知
- 🐛 **安全修复**：validateApiKey 漏读 owner_id 字段（导致用户级启停/限流失效），已修复

### v3.18.22-18（2026-09-25）· 移动端底部导航精简
- 📱 底部导航只留 4 个核心（首页/服务商/模型/内置聊天），其他收进侧边菜单

### v3.18.22-17（2026-09-25）· 语言国际化 + UI美化 + 体验优化（自主完善）
- 🌐 **i18n 多语言引擎**：15 语言全界面翻译（菜单/底部导航/按钮/提示），切换语言立即生效并重渲染当前页；登录页也支持语言选择（记住偏好）
- 📊 **首页数据看板**：运行时长 / 可用模型数 / 健康率 / 故障转移状态 四联统计卡
- 🎨 **UI 美化**：卡片悬停上浮动效、统计卡渐变、页面切换淡入动画、骨架屏加载、在线状态呼吸灯
- 💬 **聊天打字机效果**：AI 回复逐字显示（带光标 ▍），体验更流畅
- ⚡ **测速进度条**：批量测速时显示实时进度（百分比 + 当前序号）
- 🔍 **全局搜索**：顶栏搜索框，中英文模糊匹配快速跳转页面（首页/服务商/模型/测速/聊天/密钥/路由/用量/工单/个人/设置/日志/公告/用户/关于）
- 📱 登录页语言选择器（15语言，记住选择）
